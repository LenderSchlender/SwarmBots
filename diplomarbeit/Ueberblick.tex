...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        node distance=3cm,
        auto,
        robot/.style={shape=circle, draw=black, line width=1,minimum size=2cm},
        backend/.style={{shape=rectangle, draw=black, line width=2}},
        frontend/.style={{shape=rectangle, draw=red, line width=2}},
        ]
        \node (guide) [robot] {Guide};
        \node (tamerlan) [robot, below of=guide] {Tamerlan};
        \node (bambi) [robot, below of=tamerlan] {Bambi};
        \node (backend) [backend, right of=tamerlan, node distance=4cm] {Backend};
        \node (frontend) [frontend, right of=backend, node distance=4cm] {Frontend};
        ...
        \begin{scope}[transform canvas={yshift=.5em,xshift=-.2em}]
            \path[<-]
            (guide)     edge [bend left]  	node {}     (backend)
            (tamerlan)  edge []             node {}     (backend)
            (bambi)     edge [bend right]  	node {}     (backend)
